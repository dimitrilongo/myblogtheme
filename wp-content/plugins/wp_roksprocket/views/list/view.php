...
class RokSprocket_Views_List_View implements RokSprocket_Views_IView
{
    ...
    public function __construct($instance=null)
    {
        ...
        $this->base_url = get_bloginfo('wpurl') . '/wp-admin/admin.php?page=roksprocket-list&nonce=' . $this->nonce;
        $this->edit_url = get_bloginfo('wpurl') . '/wp-admin/admin.php?page=roksprocket-edit&nonce=' . $this->nonce;
        ...
        foreach ($container['roksprocket.layouts'] as $type => $layoutinfo) {
            $css .= 'i.layout.'.$type.' {background-image: url('.get_bloginfo('wpurl') . '/wp-content/plugins/wp_roksprocket/layouts/'.$type.'/icon.png);background-position: 0 0;}';
        }
        $this->inline_css = $css;
