...
class RokSprocket_Views_Postlist_View implements RokSprocket_Views_IView
{
    ...
    public function __construct($instance = null)
    {
        ...
        //add custom posttypes for Posttypes
        if ($this->provider == 'posttype') {
            $db_post_types = get_option('post_types', array());
            $post_types = (is_string($db_post_types)) ? unserialize($db_post_types) : $db_post_types;
            foreach ($post_types as $post_type) {
                $this->post_types[] = strtolower($post_type['labels']['name']);
            }
        }

        //add custom posttypes for CPT
        ...
        $this->base_link = get_bloginfo('wpurl') . '/wp-admin/admin-ajax.php?action=roksprocket_postlist&provider=' . $this->provider . '&nonce=' . $this->nonce;
        ...
        $post_args = array();
        if ($this->search)
            $post_args['name'] = '"%' . sanitize_title($this->search) . '%"';
        if ($this->category)
        ...
        $post_args['offset'] = ($this->paged - 1) * 20;

        //retrieve page data
        $this->posts = get_posts($post_args);

        $post_args['numberposts'] = 9999;
        $post_args['offset'] = 0;

        $this->total_posts = count(get_posts($post_args));
        ...
        foreach ($this->post_types as $type) {
            $typelist .= '<option rel="' . RokCommon_URL::updateParams($this->link, array("post_type" => $type)) . '" value="' . $type . '"' . rs_selected($type, $this->post_type, false) . '>' . $type . '</option>';
        }
        $typelist .= '</select>';
        ...
        $searchbox .= '<a href="' . $this->base_link . '" id="clear-button" class="button cancel" style="float:left;clear:none;">Clear Filters</a>';
