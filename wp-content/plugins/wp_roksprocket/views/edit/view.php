...
class RokSprocket_Views_Edit_View implements RokSprocket_Views_IView
{
	...
	public function __construct($instance = null)
	{
		...
		//build the back_url
		$this->base_url = get_bloginfo('wpurl') . '/wp-admin/admin.php?page=roksprocket-edit&nonce=' . $this->nonce;
		$this->list_url = RokCommon_URL::updateParams(get_bloginfo('wpurl') . '/wp-admin/admin.php?page=roksprocket-list&nonce=' . $this->nonce, $link_args);
		$this->ajax_url = get_bloginfo('wpurl') . '/wp-admin/admin-ajax.php?action=roksprocket_ajax&nonce=' . $this->nonce;
	...
	public function initialize()
	{
		...
		$this->processModuleConfig($this->form);
		if (isset($this->data)) {
			$this->form->bind($this->data->toArray());
		}
	 ...
	 */
	protected function processModuleConfig(RokCommon_Form $form)
	{
		...
		foreach ($container['roksprocket.layouts'] as $type => $layoutinfo) {
			...
			foreach ($layoutinfo->paths as $layoutpath) {
				if (is_dir($layoutpath . '/language')) {
					load_plugin_textdomain('wp_roksprocket_layout_' . $type, ltrim(str_replace(ABSPATH, '', $layoutpath . '/language'), '/\\'));
				}
