...
<div class="articles<?php echo (!count($that->articles) ? ' no-articles': '');?>">
	<div class="article-description">
		<span class="article-instructions"></span>
		<div class="article-text">
			<h2><?php echo rc__("ROKSPROCKET_INSTRUCTIONS_TITLE"); ?></h2>
			<p><?php echo rc__("ROKSPROCKET_INSTRUCTIONS_TEXT"); ?></p>
			<p><img src="<?php echo $platforminfo->getRootUrl(); ?>/wp-content/plugins/wp_roksprocket/admin/assets/images/sample.png" class="sample" /></p>
		</div>
	</div>
	<?php
	$order = 0;
	...
	foreach ($that->articles as $article):
		...
		?>
		<?php echo RokCommon_Composite::get($that->context)->load('edit_article.php', array(
