...
class RokSprocket_Widgets_RokSprocket extends WP_Widget
{
	...
	{
        $new_options = array();
		$global_options = get_option('widget_roksprocket_options');
        if ( !empty($global_options) && !array_key_exists('_multiwidget', $global_options) ) {
            // old format, convert if single widget
            $global_options = wp_convert_widget_settings('roksprocket_options', 'widget_roksprocket_options', $global_options);
        }
	...
	function form($instance)
	{
		...
		ob_start();
		echo RokCommon_Composite::get('rs_templates.widget')->load('form.php', array('instance' => $instance, 'form'=>$form));
		echo ob_get_clean();
	}
