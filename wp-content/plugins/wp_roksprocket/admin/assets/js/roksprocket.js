...
*/
        ...
        var moofx = typeof document !== "undefined" ? require("6") : require("a");
...
*/
        ...
        var prime = require("5"), array = require("3"), string = require("7");
        ...
        var CSSAnimation = prime({
            ...
            resetCSS: function(inclusive) {
                var rules = compute(this.node), properties = rules(transitionName + "Property").replace(/\s+/g, "").split(","), durations = rules(transitionName + "Duration").replace(/\s+/g, "").split(","), equations = rules(transitionName + "TimingFunction").replace(/\s+/g, "").match(/cubic-bezier\([\d-.,]+\)|([\w-]+)/g);
...
*/
        ...
        var prime = require("5"), requestFrame = require("2").request, bezier = require("b");
			...
			onModelSuccess: function(response){
				...
				if (document.getElements('.articles [data-article-id]').length > 1){
					this.sortables = new Sortables('.articles', {
						clone: true,
						opacity: (Browser.Engine.gecko) ? 1 : 0.5,
						handle: '.handle',
			...
			events:{
				...
				success: function(response){
					...
					if (status == 'success'){
						...
						if (!document.id('id') || document.id('id').get('value') == '0'){
							RokSprocket.modal.set({
