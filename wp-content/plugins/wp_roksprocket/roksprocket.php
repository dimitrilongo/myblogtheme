...
if (!defined('ROKSPROCKET_VERSION')) define('ROKSPROCKET_VERSION', '1.8.3');
...
if (!preg_match('/project.version/', ROKCOMMON) && version_compare(preg_replace('/-SNAPSHOT/', '', ROKCOMMON), '3.0', '<')) {
    rs_force_deactivate();
    roksprocket_set_admin_message('error', __('RokSprocket needs at least RokCommon Version 3.0.  You currently have RokCommon Version ' . ROKCOMMON));
    return;
}

include_once(ROKSPROCKET_PLUGIN_PATH . '/widgets/roksprocket.php');
include_once(ROKSPROCKET_PLUGIN_PATH . '/posttypes/roksprocket.php');
include_once(ROKSPROCKET_PLUGIN_PATH . '/metaboxes/roksprocket.php');
...
if (!function_exists('roksprocket_activate')) {
    ...
    function roksprocket_activate()
    {
        ...
        if (!array_key_exists('roksprocket', $container_configs)) {
    ...
    */
    function roksprocket_mootools_remove()
    {
        ...
        if ((is_admin() && $pagenow == 'widgets.php' && !$gantry)
            ...
            || (is_admin() && $page == 'roksprocket-list')
            || (is_admin() && $page == 'roksprocket-edit')
            || (!is_admin())
        ) {
