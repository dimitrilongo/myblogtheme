...
{
    global $wpdb;
    if (!$wpdb->query("SHOW tables LIKE '" . $wpdb->prefix . "rokgallery_files%';")) {
...
function rokgallery_gallerymanager_ajax()
{
    ...
    echo RokCommon_Composite::get('wp_views.gallerymanager')->load('default.php', array('view' => $view));
    $content = ob_get_contents();
    ob_end_clean();

    echo RokCommon_Composite::get('wp_views.gallerymanager')->load('window.php', array('content' => $content));
...
function rokgallery_gallerypicker_ajax()
{
    ...
    RokCommon_Header::addInlineScript(RokCommon_Composite::get($view->context)->load('javascript.php', array('that' => $view)));
    echo RokCommon_Composite::get('wp_views.gallerypicker')->load('default.php', array('view' => $view));
    $content = ob_get_contents();
    ob_end_clean();

    echo RokCommon_Composite::get('wp_views.gallerypicker')->load('window.php', array('content' => $content));
    // IMPORTANT: don't forget to "exit"
    exit;
}
...
function rokgallery_postlist_ajax()
{
    ...
    echo RokCommon_Composite::get('wp_views.default')->load('postlist.php', array('view' => $view));
    $content = ob_get_contents();
    ob_end_clean();

    echo RokCommon_Composite::get('wp_views.default')->load('window.php', array('content' => $content));
    // IMPORTANT: don't forget to "exit"
    exit;
}
...
function rg_rokgallery_check()
{
    ...
    foreach ($options as $option) {
        if (strpos($option, 'rokgallery') !== false) {
            if(rg_db_check()){
                $activated = true;
            }
        }
    ...
    if (($rokgallery) && ($activated)) {
        ...
        update_option('rokgallery_activated',true);
        return true;
...
function rg_rokcommon_check()
{
    ...
    $activated = false;
    foreach ($options as $option) {
        if (strpos($option, 'rokcommon') !== false) {
            $activated = true;
        }
    }
    if (($rokcommon) && ($activated)) {
        update_option('rokcommon_installed',true);
        update_option('rokcommon_activated',true);
        return true;
    } elseif (($rokcommon) && (!$activated)) {
        rg_force_deactivate();
        update_option('rokcommon_installed',true);
        update_option('rokcommon_activated',false);
        ...
        rg_force_deactivate();
        update_option('rokcommon_installed',false);
        update_option('rokcommon_activated',false);
