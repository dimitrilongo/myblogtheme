...
<div id="rokgallerypicker">
	...
	<div id="panels">
		<div class="panel filelist">
			<div class="instructions"><?php rc_e('ROKGALLERY_PICKER_INSTRUCTIONS', '<img src="'.$that->images_path.'jinsert.png" width="16" height="16" alt="" title="" style="vertical-align:middle;" />'); ?></div>

			<ul id="gallerypicker-fileslist">
				<?php
					if (!$that->file_id){
			            foreach($that->files as $that->file):
			                echo RokCommon_Composite::get('rokgallery.gallerypicker')->load('default_file.php', array('that'=>$that));
						endforeach;
					} else {
						foreach($that->files->Slices as $that->slice):
							echo RokCommon_Composite::get('rokgallery.gallerypicker')->load('default_slices_view.php', array('that'=>$that));
						endforeach;
					}
				?>
			</ul>
				...
				<?php
					if (!$that->gallery_id){
			            foreach($that->galleries as $that->gallery):
			                echo RokCommon_Composite::get('rokgallery.gallerypicker')->load('default_gallery.php', array('that'=>$that));
						endforeach;
					} else {
		...
		<div class="panel menuitems">
            <?php echo RokCommon_Composite::get('rokgallery.gallerypicker')->load('default_menuitems.php', array('that'=>$that)); ?>
			<div class="clr"></div>
		</div>
		<?php endif; ?>
	</div>
